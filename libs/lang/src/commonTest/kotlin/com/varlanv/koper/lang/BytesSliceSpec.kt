...
class BytesSliceSpec : BaseSpec({
    should("visit the entire slice when its offset exceeds its length") {
        val slice = BytesSlice(ReadonlyBytes(byteArrayOf(0, 0, 0, 1, 2, 0)), 3, 2)
    ...
    should("skip iteration for empty slices at either end of an array") {
        for (offset in listOf(0, 3)) {
            val slice = BytesSlice(ReadonlyBytes(byteArrayOf(1, 2, 3)), offset, 0)
    ...
    should("allocate an independent copy of only the slice contents") {
        ...
        val slice = BytesSlice(ReadonlyBytes(array), 1, 2)
    ...
    should("compare contents regardless of backing array and offset") {
        val first = BytesSlice(ReadonlyBytes(byteArrayOf(9, -128, -1, 127, 8)), 1, 3)
        val second = BytesSlice(ReadonlyBytes(byteArrayOf(-128, -1, 127)), 0, 3)
    ...
    should("reject unequal contents lengths and unrelated values") {
        val first = BytesSlice(ReadonlyBytes(byteArrayOf(1, 2, 3)), 0, 3)
        first.equals(BytesSlice(ReadonlyBytes(byteArrayOf(1, 2, 4)), 0, 3)) shouldBe false
        first.equals(BytesSlice(ReadonlyBytes(byteArrayOf(1, 2)), 0, 2)) shouldBe false
    ...
    should("give empty slices equal contents and hash codes") {
        val first = BytesSlice(ReadonlyBytes(byteArrayOf()), 0, 0)
        val second = BytesSlice(ReadonlyBytes(byteArrayOf(1, 2)), 2, 0)
        first shouldBe second
    ...
    should("preserve zero hash codes") {
        val slice = BytesSlice(ReadonlyBytes(byteArrayOf(-31)), 0, 1)
